...
from utils.MapManager import BaseAgent, InProcessAgent, AgentState
...
class SingleAgentEnv(gym.Env):
    ...
    def __init__(
        ...
        self._ideal_revisit_delta = 100
        self._recency_reward_scale = 140
    ...
    def reset(self, seed=None, options=None):
        ...
        # Spawn agents
        self.agent_instance = InProcessAgent("agent_0", self.world_size, start_pos=self.start_poss, seed=self.seed)
    ...
    def _calculate_reward(self, state:AgentState):
        ...
        new_fuel_pixels  = float(np.sum(state.delta_vp_image[:, :, 0]))
        new_fire_pixels  = float(np.sum(state.delta_vp_image[:, :, 1]))
        seen_fuel_pixels = float(np.sum(local_view[:, :, 0]))

        rc_exploration      = self._w_exploration * 0.03 * new_fuel_pixels * vp_scaling_fac
        rc_fire_disc        = self._w_fire_discovery * 0.09 * new_fire_pixels * step_advantage * vp_scaling_fac
        rc_fuel_track       = self._w_exploration_track * 0.001 * seen_fuel_pixels * vp_scaling_fac
        rc_perimeter_reward = self._w_fire_tracking * self.fire_perimeter_alignment_reward((px, py)) * vp_scaling_fac
        rc_proximity        = self.fire_proximity_bonus(px, py) * vp_scaling_fac

        b_movement = self.movement_direction_bonus(px, py)

        fire_disc_last_visited_mean = float(
            np.mean(state.revisit_ts_map)
        )
        fire_disc_ts_delta = self._step_count - fire_disc_last_visited_mean
        if fire_disc_ts_delta >= self._ideal_revisit_delta:
            self._recency_reward_scale /= 3
        if self._recency_reward_scale != 140:
            self._recency_reward_scale = np.clip(self._recency_reward_scale * 1.25, None, 140)
        recency     = float(np.mean(state.recency_image))
        pc_recency_pen = (math.exp(recency) - 1) / (math.exp(1) - 1) * self._recency_reward_scale
